...
import { URL_HOST, EMAIL_TEST, PASSWORD_TEST } from "../src/config";
...
async function registerUser(credentials: { username: string; email: string; password: string }) {
  const api = URL_API;
...
test.describe('SignIn Component', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(URL_HOST+'/sign-in');
  });
  ...
  test('should redirect on successful login', async ({ page }) => {
    ...
    await page.waitForURL(URL_HOST+'/user-tasks'); // Ajusta la URL según sea necesario
    await expect(page).toHaveURL(URL_HOST+'/user-tasks');
  });
...
test.describe("TaskUserPage Component", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(URL_HOST + "/user-tasks");
    await page.fill('input[name="email"]', EMAIL_TEST);
    await page.fill('input[name="password"]', PASSWORD_TEST);
    await page.click('button[type="submit"]');
    await page.waitForURL(URL_HOST + "/user-tasks");
    await expect(page).toHaveURL(URL_HOST + "/user-tasks");
  });
  ...
  test("should edit an existing task", async ({ page }) => {
    ...
    test('should delete a task', async ({ page }) => {
      ...
      await expect(page.locator(`button[test-id="delete_${existingTask._id}"]`)).toBeHidden();
    });
});
...
test.describe('RegisterPage Component', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(URL_HOST + '/register');
  });
...
test.describe('E2E User Flow', () => {
  test('should register user, add task, edit task, delete task and sign out', async ({ page }) => {
    // Paso 1: Registrarse
    await page.goto(`${URL_HOST}/register`);
    ...
    await page.fill('textarea[placeholder="Descripción"]', "Descripción de la tarea editada");
    await page.fill('input[type="range"]', "75");
    await page.click('button:has-text("Guardar Cambios")');
    ...
    await expect(page).toHaveURL(`${URL_HOST}/sign-in`); 
  });
});